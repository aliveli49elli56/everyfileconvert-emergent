...
// ─── Category display config (purely presentational, not in master-registry) ──
...
const SOURCE_FORMATS = FORMAT_REGISTRY.filter(
  (f) => (CONVERSION_MATRIX[f.ext]?.length ?? 0) > 0
);
...
interface FormatDropdownProps {
  placeholder: string;
  value: string | null;
  formats: typeof FORMAT_REGISTRY;
  disabled?: boolean;
...
function FormatDropdown({
  ...
  const grouped = useMemo(() => {
    const map = new Map<FormatCategory, typeof FORMAT_REGISTRY>();
    filtered.forEach((f) => {
        ...
        <span className={`text-sm font-medium truncate ${selected ? "text-slate-800" : "text-slate-400"}`}>
          {selected ? (
            <span className="flex items-center gap-2">
              <span className={`text-xs px-1.5 py-0.5 rounded font-semibold ${CATEGORY_META[selected.category].color}`}>
              ...
              <div key={cat}>
                ...
                {items.map((f) => (
                  <button
                    ...
                    `}
                  >
                    <span className={`text-xs px-1.5 py-0.5 rounded font-bold ${CATEGORY_META[f.category].color}`}>
                      .{f.ext.toUpperCase()}
                    </span>
...
export default function FormatSelector({ locale }: { locale: string }) {
  ...
  // Target formats derived from master-registry — no hardcoding
  const targetFormats = useMemo(
    () => (fromExt ? getTargetsForSource(fromExt) : []),
    [fromExt]
  );

  const handleFromChange = (ext: string) => {
    setFromExt(ext);
    setToExt(null); // reset target when source changes
  };
  ...
  return (
    ...
    >
      ...
      <div className="bg-white/60 backdrop-blur-md border border-white/60 rounded-2xl shadow-xl p-5 sm:p-6">
        ...
        {/* Convert Now button — appears only when both selected */}
        <div
