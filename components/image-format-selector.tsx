...
const IMAGE_CONVERTER_CATEGORIES = ["image", "raw", "vector", "icon", "cad"] as const;

// Derive flat list of source extensions that have at least one conversion target
const ALL_IMAGE_SOURCES: string[] = IMAGE_CONVERTER_CATEGORIES.flatMap(
  (cat) => FORMAT_CATEGORIES[cat] ?? []
).filter((ext) => CONVERSION_MATRIX[ext] !== undefined);

// Group source formats by tier for the dropdown
const SOURCE_TIER_GROUPS = {
  popular:  ALL_IMAGE_SOURCES.filter((ext) => getFormatEntry(ext)?.tier === "popular"),
  standard: ALL_IMAGE_SOURCES.filter((ext) => getFormatEntry(ext)?.tier === "standard"),
  advanced: ALL_IMAGE_SOURCES.filter((ext) => getFormatEntry(ext)?.tier === "advanced"),
} as const;
...
export default function ImageFormatSelector({
  ...
  const [targetFormat, setTargetFormat] = useState<string>(() => {
    ...
    const targets = CONVERSION_MATRIX[initialSourceFormat] ?? [];
    return targets[0] ?? "";
  });

  const targetOptions = CONVERSION_MATRIX[sourceFormat] ?? [];

  // Group target options by tier
  const targetTierGroups = {
    popular:  targetOptions.filter((ext) => getFormatEntry(ext)?.tier === "popular"),
    standard: targetOptions.filter((ext) => getFormatEntry(ext)?.tier === "standard"),
    advanced: targetOptions.filter((ext) => getFormatEntry(ext)?.tier === "advanced"),
  };
  ...
  const handleSourceChange = (value: string) => {
    ...
    const newTargets = CONVERSION_MATRIX[value] ?? [];
